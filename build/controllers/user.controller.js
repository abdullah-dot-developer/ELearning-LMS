...
exports.registerUser = (0, catchAsyncError_1.catchAsyncError)(async (req, res, next) => {
    try {
        ...
        if (isEmailExist) {
            return next(new ErrorHandler_1.default("An account with this email already exist.", 400));
        }
...
exports.loginUser = (0, catchAsyncError_1.catchAsyncError)(async (req, res, next) => {
    try {
        ...
        // console.log(user);
        if (!user) {
            return next(new ErrorHandler_1.default("User with this email doesn't exist!", 400));
        }
...
exports.updateAccessToken = (0, catchAsyncError_1.catchAsyncError)(async (req, res, next) => {
    try {
        ...
        const sessionData = await redis_1.redis.get(decoded.id);
        if (!sessionData) {
            return next(new ErrorHandler_1.default("No session found. Please log in.", 401));
        }
        // Parse session data (assumed to be stored in JSON format)
        const user = JSON.parse(sessionData);
        // Create new access token
        const accessToken = jsonwebtoken_1.default.sign({ id: user._id }, process.env.ACCESS_TOKEN, { expiresIn: "5m" } // Access token expires in 5 minutes
        );
        // Optionally: create a new refresh token (if the existing one is about to expire)
        const newRefreshToken = jsonwebtoken_1.default.sign({ id: user._id }, process.env.REFRESH_TOKEN, { expiresIn: "3d" } // Refresh token expires in 3 days
        );
        req.user = user;
...
exports.updatePassword = (0, catchAsyncError_1.catchAsyncError)(async (req, res, next) => {
    try {
        ...
        if (!oldPassword || !newPassword) {
            return next(new ErrorHandler_1.default("Please fill in all the fields!", 400));
        }
        const userId = req.user?._id;
        const user = await user_model_1.default.findById(userId);
        ...
        if (!isPasswordCorrect) {
            return next(new ErrorHandler_1.default("The old Password you entered is incorrect!", 400));
        }
